  ...
  if (type === "date") {
    return (
      <div className="flex flex-col my-2">
        <label
          htmlFor={name}
          className="text-[14px] font-medium pb-2 text-black"
        >
          {label}
        </label>
        <DatePicker
          ...
          onChange={(val) => {
            if (val !== null) {
              const formetDate = val.toISOString().split("T")[0];
              setDate(formetDate);
              setFieldValue(name, formetDate);
            }
          }}
        />
    ...
    return (
      <div className="flex flex-col my-2">
        ...
        <Field
          ...
          className="bg-input px-2 text-[14px] text-black py-2 rounded-[6px] outline-none active:border-[1px] focus:border-[1px] border-[#00984B]"
          readOnly={disabled}
        />
        <div className="text-[red] text-[13px] my-[2px]">
          <ErrorMessage name={name} />
